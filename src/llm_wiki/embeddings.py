    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        with httpx.Client(timeout=120.0) as client:
            ...
            for text in texts:
                ...
                if vec is None:
                    raise RuntimeError(
                        f"Ollama returned no embedding for text: {text[:50]}"
                    )
...
class McpEmbeddingProvider(EmbeddingProvider):
    ...
    async def _embed_async(self, texts: list[str]) -> list[list[float]]:
        try:
            ...
            raise ImportError(
                "mcp package is required for MCP embedding provider"
            ) from exc
        ...
        async with (
            client_cm as (read_stream, write_stream),
...
def create_provider(config: dict[str, Any]) -> EmbeddingProvider | None:
    ...
    if provider_type == "openai":
        ...
        if not api_key:
            raise ValueError(
                "OpenAI embedding provider requires 'embedding.openai.api_key'"
            )
