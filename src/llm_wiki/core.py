...
class WikiManager:
    ...
    def _resolve_new_page_path(self, title: str) -> Path:
        ...
        for suffix in range(100):
            candidate = self.wiki_dir / (f"{stem}.md" if suffix == 0 else f"{stem}-{suffix}.md")
            ...
            if existing is None or existing.title == title:
                ...
                return candidate
            LOG.info("Name collision: %s held by %r; trying suffix", candidate.name, existing.title)
        raise ValueError(f"Cannot allocate a unique page filename for title: {title!r}")

    def create_page(self, title: str, content: str, frontmatter: Dict,
